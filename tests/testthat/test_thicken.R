...
x_sec <- date_seq("sec")
equal_dist <- c(
  as.POSIXct("2014-01-01 23:00:00"),
  as.POSIXct("2014-01-02 01:00:00")
)

df_with_one_date <- data.frame(
...
test_that("thicken gives informed error when start_val is wrong class", {
  expect_error(thicken(x_month,
    start_val = "2017-01-01",
...
test_that("thicken gives correct ouput when x is a df", {
  X <- data.frame(
    day_var = seq(as.Date("2016-01-01"), as.Date("2016-12-31"), by = "day"), # nolint
...
Returned dataframe contains original observations, with NA values for d and d_week."
  ...
  expect_equal(
    sw(thicken(coffee_two_nas, "week"))$d_week,
...
test_that("add_na_to_thicken unit tests", {
  ...
  expect_equal(
    add_na_to_thicken(thickened_date, 2),
    ymd(c(20180101, NA, 20180101))
  )
  expect_equal(
    add_na_to_thicken(thickened_datetime, 2),
    ymd_h(c("20180101 01", NA, "20180101 04"))
  )
  expect_equal(
    add_na_to_thicken(thickened_date, c(2, 4)),
    ymd(c(20180101, NA, 20180101, NA))
  )
  expect_equal(
    add_na_to_thicken(thickened_datetime, c(2, 4)),
...
test_that("thicken will return a data frame when drop = TRUE", {
  ...
  x_ret <- data.frame(time_stamp_hour = ymd_hms(c(
    "2016-07-07 09:00:00",
    "2016-07-07 09:00:00",
    "2016-07-09 13:00:00",
    "2016-07-10 10:00:00"
...
test_that("ties_to_earlier works with rounding down regular", {
  expect_equal(
    thicken(x, "hour", ties_to_earlier = TRUE)$dt_hour,
...
test_that("ties_to_earlier works with rounding up regular", {
  expect_equal(
    thicken(x, "hour", rounding = "up", ties_to_earlier = TRUE)$dt_hour,
...
test_that("ties_to_earlier works with rounding down ties on edges", {
  expect_equal(
    thicken(x[2:3, , drop = FALSE], "hour", ties_to_earlier = TRUE)$dt_hour,
...
test_that("ties_to_earlier works with rounding up ties on edges", {
  expect_equal(
    thicken(x[1:2, , drop = FALSE], "hour", rounding = "up", ties_to_earlier = TRUE)$dt_hour,
