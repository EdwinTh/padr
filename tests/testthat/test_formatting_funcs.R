...
test_that("int_to_secs gives the correct result", {
  sec_day <- 60 * 60 * 24
  expect_equal(
    int_to_secs(list(interval = "year", step = 2)),
    sec_day * 365 * 2
  )
  expect_equal(
    int_to_secs(list(interval = "quarter", step = 1)),
    sec_day * 365 / 4
  )
  expect_equal(
    int_to_secs(list(interval = "month", step = 3)),
    sec_day * 365 / 12 * 3
  )
  expect_equal(
    int_to_secs(list(interval = "week", step = 8)),
    sec_day * 7 * 8
  )
  expect_equal(
    int_to_secs(list(interval = "day", step = 1)),
    sec_day
  )
  expect_equal(
    int_to_secs(list(interval = "hour", step = 100)),
    360000
  )
  expect_equal(
    int_to_secs(list(interval = "min", step = 10)),
    600
  )
  expect_equal(
    int_to_secs(list(interval = "sec", step = 2)),
    2
  )
})
...
test_that("find_next_val helper function", {
  expect_equal(
    find_next_val(span_date(2016, 2018), 365),
    span_date(2017, 2019)
  )
  expect_equal(
    find_next_val(span_time(2016, 2018), 365 * 24 * 3600),
    span_time(2017, 2019)
  )
  expect_equal(
    find_next_val(span_date(201601, 201603), 31),
    span_date(201602, 201604)
  )
  expect_equal(
    find_next_val(span_time(201601, 201603), 31 * 24 * 3600),
    span_time(201602, 201604)
  )
  expect_equal(
    find_next_val(span_date(201601, len_out = 3, by = "week"), 7),
    span_date(20160108, len_out = 3, by = "week")
  )
  expect_equal(
    find_next_val(span_time(201601, len_out = 3, by = "week"), 7 * 24 * 3600),
    span_time(20160108, len_out = 3, by = "week")
  )
  non_eq_days <- span_date(20160101, 20160104)[c(1, 2, 4)]
  expect_equal(
    find_next_val(non_eq_days, 1),
...
test_that("format_interval formatting", {
  ...
  expect_equal(
    fse(x2, start_format = "%F %H", end_format = "%H", sep = "-"),
...
test_that("format_interval end _offset works", {
  ...
  expect_equal(
    fse(x2, start_format = "%F %T", end_offset = 60),
    c(
      "2016-01-01 00:00:00 2016-01-01 00:59:00",
...
test_that("format_interval works with different timezones", {
  dt <- span_time("20170801 00",
    by = "3 hour", len_out = 3,
    tz = "EST"
  )
  expect_equal(
    fse(dt, start_format = "%H"),
    c("00 03", "03 06", "06 09")
  )
})
