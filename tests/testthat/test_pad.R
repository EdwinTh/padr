...
test_that("break_above prevents large output", {
  ...
  expect_equal(nrow(pad(large_df_grp,
    interval = "hour", group = "grp",
    break_above = 0.0702
  )), 70128)
})
...
test_that("gives correct output when end_val and/or start_val are specified, date", {
  ...
  expect_equal(
    pad(x, end_val = ymd(20160104), interval = "day")$tm,
    c(x$tm, ymd(20160103), ymd(20160104))
  )
  expect_equal(
    pad(x, start_val = ymd(20160101), end_val = ymd(20160104))$tm,
...
test_that("gives correct output when start or end with datetime range", {
  ...
  expect_equal(
    pad(x, start_val = ymd(20170201))$dt,
    seq(ymd(20170201), ymd(20180101), by = "month")
  )
  expect_equal(
    pad(x, end_val = ymd(20170601))$dt,
    seq(ymd(20170101), ymd(20170601), by = "month")
  )

  x_grp <- data.frame(
    dt = ymd(20170101, 20170601, 20170201, 20170501),
    id = c(1, 1, 2, 2),
    x = 1:4
  )
  expect_equal(
    pad(x_grp, group = "id", start_val = ymd(20170401))$dt,
    ymd(20170401, 20170501, 20170601, 20170401, 20170501)
  )
  expect_equal(
    pad(x_grp, group = "id", end_val = ymd(20170301))$dt,
...
test_that("grouping works with irregular colnames", {
  x <- dplyr::tibble(
    `group col` = rep(letters[1:2], each = 3),
...
test_that("the by arguments works, both in pad and pad_single", {
  ...
  two_var <- one_var
  two_var$x_year2 <- two_var$x_year
  ...
  expect_equal(
    pad(one_var_grps, by = "x_year", group = "grp", interval = "year")$x_year,
    rep(check_val, 2)
  )
...
test_that("pad works correclty when start is after or end is before range", {
  ...
  x_sing_2 <- x_grp[3:4, ]
  expect_error(
    pad(x_sing_1, start_val = as.Date("2017-08-01")),
    "start value is larger than the end value."
  )
  expect_error(
    pad(x_sing_1, end_val = as.Date("2017-03-01")),
    "start value is larger than the end value."
  )
  ...
  x_start_val <- sw(pad(x_grp,
    group = "grp", end_val = as.Date("2017-07-01"),
    interval = "month"
  ))
  expect_equal(x_sing_1_pad, x_start_val)

  x_sing_2_pad <- pad(x_sing_2, group = "grp", interval = "month")
  x_end_val <- sw(pad(x_grp,
    group = "grp", start_val = as.Date("2017-09-01"),
    interval = "month"
  ))
...
test_that("pad work well with day and week", {
  cross_dst <- data.frame(day = as.POSIXct(
...
test_that("The use of DSTday for the interval throws a meaningful error", {
  ...
  expect_error(pad(cross_dst, "DSTday"),
    "DSTday does not work for interval, please use 'day' instead",
    fixed = TRUE
  )
