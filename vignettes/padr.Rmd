...
After aggregating the data to a higher interval, all the observations in an interval are represented by a single point in time. This either the first (rounding down) or the last (rounding up) datetime point of the interval. For the moment is not the aim of `padr` to introduce new S3 classes (as `ts` and `zoo` do), so the datetime variables are left as they are. However, from from v.0.4.0 two functions are offered to reformat the datetime variable. They might give a better representation of the data when presenting them in a table or graph. First of all, `center_interval` will move the time point to the center of the interval. This would give a better representation in point, line and bar graphs.
