...
Raw data containing date or time variables is often not ready for direct analysis. Data might be recorded on too granualar a level, and it needs to be aggregated to a higher time unit before we can do any meaningful analysis. Furthermore there are usually no records when there are no events. While this is logical from a data recording point of view, it is often problematic when doing analysis like building plots or calculating a moving averag. Take the following example of count data.
...
## Any suggestions?
Found a bug? Ideas for improving or expandig `padr`. Your input is highly appreciated. The code is maintained at https://github.com/EdwinTh/padr and your most welcome to file and issue or do a pull request.
