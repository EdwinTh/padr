...
Regarding daylight savings time and different time zones some explicit choices had to be made. They are explained here, so the user knows what to expect when using the `padr` funcitons. The last paragraph provides detailed information about the workings of `thicken`.

## Daylight savings time
The `padr` package relies heavily on the base R functions `seq.Date` and `seq.POSIXt` to span the date and datetime variables for the `thicken` and `pad` functions. In the latter, choices had to be made about ignoring daylight savings time or not. Take the following example.
...
This is the default spanning behavior of the seven intervals. The first five will be of class `Date`, the last two will be of class `POSIXct`.

* **year** will start and end on January 1st
* **quarter** will start and end on one of the following: January 1st, April 1st, July 1st, or October 1st.
* **month** will start and end on the first day of a month.
* **week** will start and end on a Sunday.
* **day** will start and end on any given day.
* **hour** will start and end on the full hour, with minute and second being zero.
* **minute** will start and end on the full minute, with second being zero.
