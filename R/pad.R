...
pad <- function(x,
  ...
  x_NA_list <- check_for_NA_pad(
    x, dt_var_info_original$dt_var,
    dt_var_name
  )
  x <- x_NA_list$x
  x_NA <- x_NA_list$x_NA
  ...
  return_frame <- to_original_format(
    return_frame,
    group,
    dt_var_name,
    original_data_frame
...
warning_no_vary <- function(msg_second_part) {
  function(x) {
    ...
    if (any(start_equal_to_end)) {
      ...
      warning(sprintf(
        "datetime variable does not vary for %d of the groups, %s", # nolint
        not_varying, msg_second_part
      ), call. = FALSE)
    }
  }
}
...
check_invalid_start_and_end <- function(x) {
  ...
  if (total_invalid > 0) {
    warning(sprintf(
      "%d group(s) for which the start value is larger than the end value, removed from return.", # nolint
      total_invalid
    ), call. = FALSE)
  }
  x <- x[!x$invalid, ]
